...
async function fetchBlogPosts(): Promise<BlogPost[]> {
  try {
    ...
    // Skip the first "# Blog Posts" heading
    ...
    sections.forEach((section, index) => {
      ...
      // Improved tag parsing - check if the line exists and if it starts with 'tags:'
      let tags: string[] = [];
      if (lines[4] && lines[4].startsWith('tags:')) {
        tags = lines[4].replace('tags:', '').split(',').map(tag => tag.trim());
      }
    ...
    console.log('Parsed blog posts:', posts.length, posts.map(p => p.title));
    return posts;
  } catch (error) {
